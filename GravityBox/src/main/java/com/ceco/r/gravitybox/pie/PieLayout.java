...
import com.ceco.r.gravitybox.pie.PieController.Position;
...
public class PieLayout extends FrameLayout implements View.OnTouchListener {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mPosition == Position.LEFT || mPosition == Position.RIGHT) {
            mCenter.x = mPadding + (getWidth() - 2 * mPadding) * mPosition.FACTOR;
            ...
            mCenter.y = mPadding + (getHeight() - 2 * mPadding) * mPosition.FACTOR;
        }
